...
arcpy.management.CreateFeatureclass(ws,os.path.split(outshape)[1],'POINT')

#Create Fields for attributes 
arcpy.management.AddField(outshape, 'propa', 'DOUBLE', 7, 3)
arcpy.management.AddField(outshape, 'propb', 'DOUBLE', 7, 3)

#Creates an insert cursor from the blank shapefile
icurs = arcpy.da.InsertCursor(outshape,['shape@', 'propa', 'propb'])
...
for item in infile_lines:
    ...
    icurs.insertRow([pnt, float(it_sp[2]), float(it_sp[3])])
